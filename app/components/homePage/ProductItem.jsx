...
const ProductItem = React.memo(({ product, index, currentImageIndex, direction, handleTouchStart, handleTouchEnd }) => {
  return (
    <div className="group bg-[#FFFCF7] rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 relative">
      <Link href={product.link}>
        ...
        >
          ...
          />
          {product.discount > 0 && (
            <div className="absolute top-0 right-0 bg-red-600 text-cream py-1 px-2 rounded-md text-sm font-semibold">
              {product.discount}%
            </div>
          )}
        </div>
